...
* `site/` = `https://firmware.ffnef.de/stable/21_dias.key/site` (site.conf,
  ...
  * `modules`: Feed `nodeplacer` = `file:///home/adorfer/projekte/freifunk/packages/nodeplacer`
  ...
  * `image-customization.lua`: `'nodeplacer'` in der Paketliste.
...
Paket `nodeplacer_0.1-1_x86_64.ipk` (13 KB) aus dem Container-Build per
